...
def normalizar_modalidade(modalidade_str):
    ...
    # Remove "Modalidade" se for só isso
    if modalidade_str == "Modalidade":
        return None

    # Normaliza caracteres com encoding errado
    replacements = {
        "Ã©": "é",
        "Ã¡": "á",
        "Ã­": "í",
...
def criar_lancamento(data, valor, modalidade_nome, dry_run=True):
    ...
    if modalidade_nome and ("Crediário" in modalidade_nome or "Crediario" in modalidade_nome):
        # Exceção: "Recebimento Crediario" NÃO deve ser ignorado
    ...
    if modalidade_info["id"] == "PRECISA_DO_ID":
        print(f"  ⚠️  ID NÃO CONFIGURADO: '{modalidade_nome}'")
        return {"status": "error", "reason": "id_nao_configurado"}
...
def processar_csv(arquivo_csv, dry_run=True):
        ...
        for i in range(0, min(len(linha), len(datas) * 2), 2):
            ...
            if valor and modalidade:
                ...
                if resultado["status"] == "success" or resultado["status"] == "dry_run":
                    ...
                    total_ignorado += 1
                elif resultado.get("reason") == "id_nao_configurado":
                    total_id_faltando += 1
                else:
                    total_erro += 1
    ...
    print(f"  IDs faltando: {total_id_faltando}")
...
if __name__ == "__main__":
    ...
    if dry_run:
        print("\n💡 Para executar de verdade, rode: python import_lancamentos_v2.py execute")
